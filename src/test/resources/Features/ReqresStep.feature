...
#      |2000   |
  ...
  Scenario: POST create new user with valid json
    ...
    Then Should return status code 201 created And Success create new user with valid json schema
    And Validation Json schema
  Scenario Outline: Put update user
    Given Put edit user id <id> with valid json schema
    When Send request put update user
    Then Should return status code 200 And Success update user with valid json schema
    And Validation Json schema
  Examples:
    |id|
    |2 |
